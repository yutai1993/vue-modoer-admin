...
export default {
  ...
  props: {
    ...
    plugins: {
      type: [String, Array],
      default: 'print preview searchreplace autolink directionality emoticons indent2em visualblocks visualchars fullscreen image axupimgs link media template code codesample table charmap hr pagebreak nonbreaking anchor insertdatetime advlist lists wordcount textpattern autosave '
    },
    /* toolbar: { // 这种方式有抽屉
        type: [String, Array],
        default: 'fullscreen undo redo restoredraft | cut copy paste pastetext | forecolor backcolor bold indent2em italic underline strikethrough link anchor | alignleft aligncenter alignright alignjustify outdent indent | styleselect formatselect fontselect fontsizeselect | bullist numlist | blockquote subscript superscript removeformat | table image axupimgs media charmap emoticons hr pagebreak insertdatetime print preview | code selectall searchreplace visualblocks | indent2em lineheight formatpainter'
      }, */
    toolbar: { // 这种全部显示
      type: Array,
      default: () => {
        return ['fullscreen formatselect fontselect fontsizeselect undo redo restoredraft cut copy paste pastetext forecolor backcolor bold indent2em italic underline strikethrough link anchor alignleft aligncenter alignright alignjustify outdent indent bullist numlist blockquote subscript superscript removeformat table image axupimgs media charmap emoticons hr pagebreak insertdatetime print preview code selectall searchreplace visualblocks indent2em lineheight formatpainter']
      }
    }
  },
  ...
  data () {
    return {
      init: {
        ...
        emoticons_database_url: '/tinymce/emoticons/js/emojis.js', // 表情
        ...
        images_upload_handler: (blobInfo, success, failure) => {
          if (blobInfo.blob().size / 1024 / 1024 > 2) {
            ...
            axios.post('/api/uploadImage', params, config).then(res => {
              console.log(res)
              if (res.code === 200) {
                success(res.imageUrl) // 上传成功，在成功函数里填入图片路径
              } else {
                failure('上传失败')
              }
            }).catch(() => {
        ...
        file_picker_callback: (callback, value, meta) => {
          const filetype = '.pdf, .txt, .zip, .rar, .7z, .doc, .docx, .xls, .xlsx, .ppt, .pptx, .mp3, .mp4, .jpg' // 限制文件的上传类型
          ...
          inputElem.onchange = () => {
            ...
            if (file.type.slice(0, 5) == 'video') { // 判断文件类型
              upurl = '/api-upload/uploadTxVideo' // 后端媒体连接
            } else {
              upurl = '/api-upload/upload' // 后端图片连接
            }
