...
export default {
  ...
  mounted () {
    /* 默认tag */
    this.$store.commit('tagsView/TAGDEFAULT', this.$router)
    /* 刷新时创建选中的tag view */
  ...
  methods: {
    ...
    tagsNextHandle () {
      const tagsContainerWidth = Math.ceil(this.$refs.tagsContainer.getBoundingClientRect().width)
      const tagsScrollItemsWidth = Math.ceil(this.$refs.tagScrollItems.getBoundingClientRect().width)
      const currentTagScroll = Math.abs(parseInt(this.tagScroll))
      if (tagsScrollItemsWidth < tagsContainerWidth) return
      // 剩余路程
      const left = tagsScrollItemsWidth - tagsContainerWidth - currentTagScroll
      if (Math.floor(left) === 0) return
      const x = left > tagsContainerWidth * 2 ? (currentTagScroll + tagsContainerWidth) : (tagsScrollItemsWidth - tagsContainerWidth)
      this.tagScroll = -x + 'px'
    },
    ...
    tagsPrevHandle () {
      const tagsContainerWidth = Math.ceil(this.$refs.tagsContainer.getBoundingClientRect().width)
      const tagsScrollItemsWidth = Math.ceil(this.$refs.tagScrollItems.getBoundingClientRect().width)
      const currentTagScroll = parseInt(this.tagScroll)
      if (tagsScrollItemsWidth < tagsContainerWidth) return
      if (currentTagScroll === 0) return
      const x1 = (currentTagScroll + tagsContainerWidth) > 0 ? 0 : (currentTagScroll + tagsContainerWidth)
